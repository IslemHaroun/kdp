...
@trace
def populate_modules_in_parallel(wizard_controller, openai_client):
    """Trigger parallel OpenAI API calls for each module in Step 3."""
    def call_module_api(module):
        prompt_text = module["prompt"].value
        size = module["size"].value
        prompt_with_size = f"{prompt_text} Size: {size}."
        response = openai_client.get_response(prompt_with_size, max_tokens=150)
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        futures = [executor.submit(call_module_api, module) for module in wizard_controller.dynamic_modules]
